...
function ParseSubmission(body, id) {
    ...
    return {
        id,
        url: 'https://www.furaffinity.net/view/' + id,
        title: header[1].childNodes[3].childNodes[1].childNodes[0].rawText,
        posted: (/title="([\s\S]+?)"/g.exec(stats[6].rawAttrs) || '')[1],
        // @ts-ignore
        rating: Enums_1.Rating[(/alt="([\s\S]+?) rating"/g.exec(stats[55].childNodes[1].rawAttrs) || '')[1]],
        author: {
            url: 'https://www.furaffinity.net/user/' + header[1].childNodes[3].childNodes[3].childNodes[0].rawText,
            name: header[1].childNodes[3].childNodes[3].childNodes[0].rawText
        },
        content: {
            // @ts-ignore
